...
- [x] **T-037** Прогнать **Сценарий 1** из `quickstart.md` на десктопе мышью — закрыть AC-1, AC-2, AC-3, AC-4, AC-5, AC-6, AC-10, AC-12. (AC-1, AC-2, AC-3, AC-4, AC-5, AC-6, AC-10, AC-12)
- [x] **T-038** Прогнать **Сценарий 2** (возврат карточки на занятую клетку и за пределы поля) — закрыть AC-7, AC-8. (AC-7, AC-8)
- [x] **T-039** Прогнать **Сценарий 3** (локализация ru ↔ en на сцене `play`) — закрыть AC-11. (AC-11)
- [ ] **T-040** Прогнать **Сценарий 4** на iPhone SE (или iPhone 12) в ландшафтной ориентации через staging URL: жест работает, страница не скроллится, hitbox удобный. Закрыть AC-9, NFR-1, NFR-2, NFR-6. (AC-9) — отложено до staging URL после деплоя.
- [x] **T-041** Прогнать **Сценарий 5** (ресайз окна десктопа во время и после установки защитников) — подтвердить NFR-3 и NFR-4. (NFR-3, NFR-4)
- [x] **T-042** Сверить итоговое поведение с `spec.md §5`, отметить все 12 AC и пройденные ревью-чеклисты в `spec.md §11`. (AC-1…AC-12)
...
- Всего задач: 46
- Выполнено: 41 / 46 (Setup, Contract, Content, Core, UI, Integration кроме мобилы)
- Открытые: T-040 (мобила на staging), T-043…T-046 (Deploy).
- Текущий блокер: —
